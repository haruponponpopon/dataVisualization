...
function distQuant(data, id){
    ...
    function getPointsMulti(_, i) {return _.map(function(d,j){ return {x:j, y:(choice_legend[i] ? d[i] : 0 )};});	}
...
"#B1EDA3","#71D2E1","#A954DC","#91DF6E","#CB6429","#D64ADC"];
    ...
    function transitionClick(type){
		...
		var layers = d3.layout.stack().offset("zero")(data.dP.map(function(d, i){ return getPointsMulti(data[type], i);}));
		var svg = d3.select("#"+id).select("."+type);
