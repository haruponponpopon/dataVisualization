...
function InitGraphPre(data, id){
    // add title.
    d3.select("#"+id).append("h3").style("letter-spacing", "0.5em").text("都道府県ごとの店舗数遷移");
...
function updateGraphPre(data, id) {
	if (current_pre == undefined) {
		d3.select("#"+id).select("h3").style("letter-spacing", "0.5em").text("都道府県ごとの店舗数遷移");
	}
	else {
		d3.select("#"+id).select("h3").style("letter-spacing", "0.5em").text(current_pre + "の店舗数遷移");
	}
    pull_choice_data_pre();
    var type = "dist";
    var ganre_list;
